...
begin
  ...
  into job_record
  from public.jobs j
  where j.id = quote_record.job_id
  for update;
  ...
  if quote_record.status = 'sent' then
    update public.job_quotes jq
    set status = 'superseded'
    where jq.job_id = quote_record.job_id
      and jq.id <> quote_record.id
      and jq.status in ('draft', 'sent');
  ...
  on conflict on constraint payments_job_id_key do nothing;
